...
RUN /elabftw/composer.phar install --prefer-dist --no-progress --no-suggest --no-dev -a && echo "network-timeout 600000" > .yarnrc && yarn install --pure-lockfile && yarn run buildall && rm -rf node_modules && yarn cache clean && /elabftw/composer.phar clear-cache
